...
* 今天登录网站才发现`了解我们`页面样式乱了，查看之前的修改记录发现是自己挖的坑~ 当时在压缩版的样式表里修改鼠标样式时多删了一个后大括号~ 修改文件后可别忘了及时查看效果~
...
* 文章阅读页设计参考 Clean Blog 风格，减少不必要的视觉阻碍，阅读体验更加顺畅。
* 自从多说停止服务后友言评论系统加载十分缓慢与不稳定，考虑到Disqus对与国内用户体验不太友好，经多次测试决定将评论系统更改为 来必力，提高用户体验。
