...
function stepFadeOut() {
    var oArticleItems = $$(".article-item"),
        offsetStack = []

    Array.prototype.forEach.call(oArticleItems, function (value, idx) {
        offsetStack.push({
            target: value,
            offset: value.offsetTop - 450
        });
    })
    ...
    function show () {
        if(!offsetStack.length) {
            window.removeEventListener("scroll" , arguments.callee);
        }
        
        var top = document.body.scrollTop || document.documentElement.scrollTop;

        if(offsetStack[0] && top > offsetStack[0].offset) {
            offsetStack[0].target.classList.remove("article-item--hidden");
            offsetStack.shift();
            return show(); // 当在底部刷新的时候，递归将全部item显示
        }
    }
}
